...
import time  # Only if using simulated progress
...
def main():
    ...
    # Home Page
    if choice == "🏠 Home":
        st.header("🚀 Travel Agent Video Summary Assistant ")
        st.markdown("---")
        st.markdown("""
...
""")
    ...
    # Travel Agent Page
        ...
        destination = st.text_input("Enter travel destination:", DESTINATION)
        preferences = st.multiselect("Select your preferences:", PREFERENCES, default=PREFERENCES)
        ...
        # Generate Itinerary
        if st.session_state['videos_df'].empty == False and 'Summary' in st.session_state['videos_df'].columns:
            if st.sidebar.button("🗓️ Generate Itinerary"):
                with st.spinner("🗓️ Generating itinerary..."):
                    # Add an input for trip duration
                    duration = st.number_input("Enter trip duration (in days):", min_value=1, max_value=30, value=3, step=1)
    ...
    # Footer
    st.markdown("---")
    st.markdown("© 2024 Travel Guide Video Summarizer by Team-206. All rights reserved. 🛡️")
